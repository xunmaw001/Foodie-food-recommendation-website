          ...
          <el-form-item class="input" label="规格" prop="guige">
              <el-input v-model="ruleForm.guige" 
                placeholder="规格" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="介绍" prop="jieshao">
          <file-upload
          tip="点击上传介绍"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.jieshao?ruleForm.jieshao:''"
          @change="jieshaoUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.jieshao" label="介绍" prop="jieshao">
            <el-button type="text" size="small" @click="download(ruleForm.jieshao)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `zaixianshangcheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zaixianshangchengCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
				...
				this.addEditUploadStyleChange()
                                            ...
                                            jieshaoUploadChange(fileUrls) {
                this.ruleForm.jieshao = fileUrls;
				this.addEditUploadStyleChange()
            },
                                                	addEditStyleChange() {
	  this.$nextTick(()=>{
